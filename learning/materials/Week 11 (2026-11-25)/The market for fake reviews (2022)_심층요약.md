...
### 2.1 원문 Table 2: 리뷰를 사려던 상품은 경쟁상품과 어떻게 달랐나

앞에서 인용한 누적 리뷰 중앙값 45개와 평균 183.08개는 원문 Table 2의 한 행이다. 1장에서 제기한 cold-start 설명을 검토하려면 모집 상품이 같은 시장의 다른 상품보다 얼마나 젊은지, 리뷰가 얼마나 적은지, 평점과 가격은 어땠는지를 함께 봐야 한다. 아래 표는 원문 Table 2(PDF 파일 6쪽, 인쇄 쪽수 473)의 7개 변수와 모든 열을 옮기고 형식만 Markdown으로 바꾼 것이다. 원문 표에는 별도 주석이 없다.

| 변수 | 집단 | Count | Mean | SD | 25% | 50% | 75% |
|---|---|---:|---:|---:|---:|---:|---:|
| Displayed rating | 모집 상품 | 1,315 | 4.40 | 0.51 | 4.10 | 4.50 | 4.80 |
| | 비교상품 | 203,480 | 4.23 | 0.59 | 4.00 | 4.30 | 4.60 |
| Number of reviews | 모집 상품 | 1,425 | 183.08 | 493.47 | 10.00 | 45.00 | 167.00 |
| | 비교상품 | 203,485 | 451.38 | 2,619.02 | 13.00 | 59.00 | 250.00 |
| Price (달러) | 모집 상품 | 1,425 | 33.36 | 44.96 | 15.99 | 23.99 | 34.99 |
| | 비교상품 | 236,542 | 44.69 | 154.80 | 12.99 | 20.99 | 39.95 |
| Sponsored | 모집 상품 | 1,425 | 0.13 | 0.33 | 0.00 | 0.00 | 0.00 |
| | 비교상품 | 236,542 | 0.11 | 0.32 | 0.00 | 0.00 | 0.00 |
| Keyword position | 모집 상품 | 1,425 | 21.41 | 16.11 | 8.00 | 16.00 | 33.00 |
| | 비교상품 | 236,542 | 28.18 | 17.32 | 13.00 | 23.00 | 43.00 |
| Age (days) | 모집 상품 | 1,305 | 229.82 | 251.12 | 77.00 | 156.00 | 291.00 |
| | 비교상품 | 153,625 | 757.84 | 797.14 | 257.00 | 466.00 | 994.00 |
| Sales rank | 모집 상품 | 1,300 | 73,292.27 | 151,236.36 | 7,893.25 | 26,200.50 | 74,801.50 |
| | 비교상품 | 5,647 | 89,926.06 | 323,028.92 | 5,495.00 | 21,610.00 | 72,563.50 |

*원문 집단 이름은 "Fake review products"와 "All products"다. 여기서는 각각 모집 상품, 비교상품으로 옮겼다.*

변수마다 두 행이 짝을 이룬다. 위 행은 Facebook 그룹에서 리뷰 모집이 관찰된 상품이고, 원문 본문(PDF 5-6쪽)에 따르면 평점과 리뷰 수는 첫 Facebook 게시물 시점의 값이다. 아래 행의 "All products"는 이름과 달리 Amazon 전체 상품을 뜻하지 않는다. 모집 상품과 같은 키워드로 검색했을 때 같은 검색결과 페이지에 나타난 경쟁상품이다. Count는 그 변수의 값이 확보된 상품 수라서 행마다 다르다. 특히 판매순위의 비교상품은 5,647개뿐이어서 20만 개가 넘는 다른 행과 표본이 크게 다르다. SD는 상품 사이의 흩어짐을 나타내는 표준편차이며 평균의 표준오차와 다르다. 25%, 50%, 75% 열은 분위수이고 50%가 중앙값이다.

나이 행부터 보면 모집 상품의 중앙값은 156일, 비교상품은 466일이다. 30일을 한 달로 치면 약 5.2개월과 15.5개월로, 원문 본문의 "약 5개월 대 15개월"과 일치한다. 그러나 모집 상품의 평균 나이도 229.82일(약 7.7개월)이고 25% 분위수가 77일이다. 원문은 약 1,500개 상품 가운데 상장 첫 달에 리뷰를 모집한 상품이 94개뿐이라고 적는다. 막 출시된 상품만 리뷰를 사는 시장으로 보기는 어렵다.

평점은 모집 상품 쪽이 더 높다. 평균 4.40 대 4.23, 중앙값 4.50 대 4.30이다. 나쁜 평판을 덮기 위해 리뷰를 산다는 단순한 설명도 이 수치와 잘 맞지 않는다. 리뷰 수는 중앙값 45 대 59로 모집 상품이 적지만 격차가 크지 않다. 평균 183.08과 표준편차 493.47은 리뷰가 1,000개를 넘는 소수 상품이 평균을 끌어올린다는 뜻이다. 비교상품의 평균 451.38과 표준편차 2,619.02에서는 이 꼬리가 더 길다.

가격과 판매순위에서는 평균과 중앙값의 비교 방향이 반대로 나온다. 가격 평균은 모집 상품 33.36달러, 비교상품 44.69달러이지만 중앙값은 23.99달러 대 20.99달러로 모집 상품이 더 비싸다. 원문은 비교상품 쪽에 고가 상품이 훨씬 많아 평균이 올라갔다고 설명한다. 판매순위는 숫자가 작을수록 잘 팔린다. 평균은 모집 상품이 73,292로 더 좋아 보이지만 중앙값은 26,200.5 대 21,610으로 모집 상품이 약간 덜 팔린다. 원문도 모집 상품의 판매가 "약간 낮지만 차이는 작다"고 쓴다. 치우친 분포에서는 중앙값을 먼저 읽는 편이 안전하다.

Sponsored는 광고 여부를 나타내는 0/1 변수의 평균이므로 0.13은 모집 상품의 약 13%가 sponsored listing을 샀다는 뜻이다. 비교상품은 11%다. 75% 분위수까지 모두 0.00인 것은 광고 상품의 비중이 25%보다 작기 때문이다. Keyword position은 모집 상품이 평균 21.41, 비교상품이 28.18이다. 원문 표에는 정의 주석이 없지만 검색결과 안의 노출 순서로 읽히며, 그렇다면 모집 상품이 페이지의 약간 앞쪽에 있었다는 뜻이 된다.

이 표로 말할 수 있는 것은 리뷰를 사려던 상품이 경쟁상품보다 젊고 리뷰가 조금 적지만 평점은 높고, 리뷰가 거의 없는 상품은 소수라는 점이다(원문 본문 기준 리뷰 0개 상품은 약 8%). 이 표만으로 말할 수 없는 것도 있다. 첫째, 두 집단 차이가 통계적으로 유의한지는 표에 검정통계량이 없어 판단할 수 없다. 둘째, 모집 상품의 높은 평점은 관찰되지 않은 과거의 리뷰 구매 결과일 수 있다고 원문 스스로 지적한다. 셋째, 비교상품은 리뷰 구매가 "관찰되지 않은" 상품일 뿐 정직한 상품이라는 보증은 없다. 넷째, 이 기술통계는 누가 리뷰를 사는지 보여 줄 뿐, 리뷰 구매가 판매에 미친 효과와는 별개의 질문이다.
...
### 7.1 원문 Table 5와 Table 6: 매칭 전후의 균형표

앞에서 통제상품이 더 오래되었고 사전 주별 평점이 낮으며 누적 리뷰가 많았다고 했다. 이 판단의 근거가 원문 Table 5이고, 매칭 후 그 차이가 얼마나 줄었는지를 보여 주는 것이 Table 6이다. 두 표는 같은 PDF 쪽(PDF 파일 16쪽, 인쇄 쪽수 483)에 같은 행 구성으로 실려 있어 아래에 나란히 합쳐 옮겼다. 행과 수치는 원문 그대로이며 열 배치만 재구성했다.

| 변수 | 매칭 전 Control | 매칭 전 Treated | 매칭 전 t | 매칭 후 Control | 매칭 후 Treated | 매칭 후 t |
|---|---:|---:|---:|---:|---:|---:|
| Age | 9.84 | 7.15 | 2.36\* | 7.78 | 7.63 | 0.10 |
| Weekly average ratings | 4.10 | 4.32 | -2.07\* | 4.07 | 4.15 | -0.48 |
| Cumulative average ratings | 4.32 | 4.43 | -1.36 | 4.34 | 4.33 | 0.07 |
| Weekly reviews | 5.21 | 5.78 | -0.33 | 5.11 | 7.24 | -0.72 |
| Cumulative reviews | 234.80 | 109.90 | 3.11\*\* | 109.48 | 124.69 | -0.39 |
| Price | 27.10 | 33.60 | -1.38 | 25.74 | 32.63 | -1.20 |
| Coupon | 0.23 | 0.26 | -0.37 | 0.24 | 0.27 | -0.30 |
| Verified | 0.92 | 0.93 | -0.60 | 0.94 | 0.95 | -0.31 |
| Number of photos | 0.25 | 0.26 | -0.15 | 0.22 | 0.24 | -0.19 |
| Category | 41.60 | 40.20 | 0.42 | 43.30 | 39.32 | 0.75 |

*원문 주석. Table 5: 처치와 통제의 평균이 같은지에 대한 t 검정이며, 평균은 [-8, -2)주 구간 수준에서 계산했다. \*p < 0.05, \*\*p < 0.01, \*\*\*p < 0.001. Table 6: PSM 가중치를 쓴 가중 t 검정이며 평균 계산 구간은 같다. Table 6에는 유의성 표시가 없다.*

각 행은 모집 시작 8주 전부터 2주 전 직전까지의 구간에서 계산한 상품 특성의 평균이다. Control은 삭제 집중기간에 모집을 시작한 상품, Treated는 그 밖의 시기에 모집한 상품이다. 표본은 본문 기준으로 매칭 전 처치 1,412개와 통제 78개, 매칭 후 처치 987개와 통제 48개이며 표 안에는 N이 적혀 있지 않다. t 열의 부호는 Control 평균에서 Treated 평균을 뺀 방향과 일치한다. 나이 행에서 9.84가 7.15보다 크고 t가 +2.36인 것이 그 예다.

매칭 전에는 세 변수에서 유의한 차이가 난다. 통제상품의 나이는 9.84로 처치상품의 7.15보다 많고(t = 2.36, p < 0.05), 사전 주별 평균평점은 4.10으로 4.32보다 낮으며(t = -2.07, p < 0.05), 누적 리뷰는 234.80개로 109.90개의 두 배가 넘는다(t = 3.11, p < 0.01). 저자들이 성향점수 모형에 이 세 변수를 넣은 이유가 바로 이 행들이다. 매칭 후에는 나이가 7.78 대 7.63(t = 0.10), 주별 평점이 4.07 대 4.15(t = -0.48), 누적 리뷰가 109.48 대 124.69(t = -0.39)로 바뀐다. 통제상품의 누적 리뷰 평균이 234.80에서 109.48로 크게 내려간 것은 가중치가 처치상품과 성향점수가 가까운 통제상품에 무게를 싣고, 공통지지 밖의 관측을 제외했기 때문이다.

모든 격차가 줄어든 것은 아니다. 주별 리뷰는 매칭 전 5.21 대 5.78에서 매칭 후 5.11 대 7.24로 오히려 벌어졌고, 가격도 25.74달러 대 32.63달러로 약 7달러 차이가 남는다. 두 행의 t는 -0.72와 -1.20으로 유의하지 않다. 매칭 후 통제상품이 48개라는 점과 가중치에 따른 유효 표본 크기도 검정력을 판단할 때 고려해야 한다. 표본이 작으면 같은 크기의 격차도 t 값이 작게 나오므로, 유의하지 않다는 결과를 두 집단이 같다는 증거로 바로 읽으면 안 된다. 원문은 표준화 평균차이처럼 표본 크기에 덜 민감한 균형 지표를 이 표에 보고하지 않는다.

단위도 조심해서 읽어야 한다. Age의 단위는 표에 적혀 있지 않다. Table 2의 나이가 일 단위였던 것과 달리 9.84, 7.15 같은 값은 월 단위로 보는 편이 자연스럽지만 원문 표에서 확인되지는 않는다. Coupon과 Verified는 0에서 1 사이의 비율이다. Category 행은 범주형 변수를 숫자로 평균한 값으로 보이며 원문에 코딩 설명이 없어 41.60이라는 크기 자체에는 해석할 의미가 없다.

이 표로 말할 수 있는 것은 매칭이 관측된 사전 특성, 특히 나이와 사전 평점과 누적 리뷰의 평균 차이를 통계적으로 구별되지 않는 수준으로 줄였다는 점이다. 말할 수 없는 것은 두 집단의 판매 추세가 같았다는 점이다. 표의 값은 [-8, -2)주 구간의 수준 평균이고 추세를 비교하지 않는다. 이 구간은 9장에서 본 -2주와 -1주의 사전 변화도 포함하지 않는다. 관찰하지 못한 상품별 수요 충격이 균형을 이루었다는 보증도 이 표에는 없다.
