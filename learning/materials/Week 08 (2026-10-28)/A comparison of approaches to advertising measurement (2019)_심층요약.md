...
### 발췌 표: 구매 결과가 있는 14개 연구의 실험 ATT와 lift (원문 Table 4)

앞에서는 Study 4 한 행만 가지고 lift의 분모를 설명했다. 같은 계산을 캠페인마다 적용하면 결과가 얼마나 달라지는지는 원문 Table 4를 봐야 알 수 있다. 이 표에 모인 RCT lift는 뒤에서 관측자료 방법을 평가할 때 기준값으로도 쓰인다. 아래 표는 원문 Table 4(PDF 파일 기준 20쪽, 인쇄 212쪽)에서 결과변수가 checkout(구매 완료 페이지 도달)인 14개 행만 옮긴 발췌다. registration 5행과 page view 3행은 생략했다. 수치는 원문 그대로이며, 원문이 5% 수준에서 유의한 값을 굵게 표시한 부분은 옮기지 않고 아래 해설에서 신뢰구간으로 대신 설명한다.

| 연구 | 노출률 (%) | 전환확률 (%): test 노출자 | 전환확률 (%): test 비노출자 | RCT ATT (%) | RCT ATT lift (%) | lift 95% 신뢰구간 (%) |
|---|---|---|---|---|---|---|
| S1 | 76 | 0.151 | 0.069 | 0.035 | 30.0 | [16, 46] |
| S2 | 48 | 0.054 | 0.014 | 0.001 | 1.3 | [−5, 8] |
| S3 | 66 | 0.260 | 0.131 | 0.021 | 8.8 | [1.1, 17] |
| S4 | 37 | 0.079 | 0.025 | 0.033 | 72.8 | [49, 103] |
| S5 | 30 | 0.055 | 0.008 | 0.045 | 449.6 | [306, 761] |
| S7 | 51 | 0.284 | 0.217 | 0.007 | 2.7 | [−0.3, 6] |
| S8 | 26 | 0.069 | 0.039 | −0.002 | −2.9 | [−21, 23] |
| S9 | 6.6 | 2.105 | 0.052 | 0.049 | 2.4 | [−0.1, 5] |
| S10 | 65 | 0.127 | 0.092 | 0.003 | 2.0 | [−11, 20] |
| S11 | 42 | 0.488 | 0.124 | 0.039 | 8.6 | [5, 13] |
| S12 | 77 | 6.403 | 2.810 | 0.078 | 1.2 | [0.2, 2] |
| S13 | 30 | 0.187 | 0.309 | −0.033 | −15.1 | [−35, 20] |
| S14 | 35 | 0.068 | 0.019 | 0.026 | 62.0 | [43, 86] |
| S15 | 81 | 1.470 | 1.175 | 0.034 | 2.4 | [0.4, 5] |

*출처: Gordon 등 (2019) Table 4 발췌, PDF 20쪽. Study 6은 구매 결과 없이 page view만 측정해 이 발췌에 없다.*

각 행은 캠페인 하나다. 노출률은 test 집단 중 실제로 광고를 본 사람의 비율로, 5절의 $p$에 해당한다. 두 전환확률 열은 모두 test 집단 내부의 값이며 노출자와 비노출자를 나눈 것이다. 통제집단의 전환율은 이 표에 없고 원문 Table 3에 있다. RCT ATT는 확률의 차이이므로 단위를 %포인트로 읽는다. %포인트를 인원수로 바꾸고 싶어질 수 있지만, 여기서는 그렇게 읽으면 안 된다. 원문 211쪽(PDF 19쪽)은 기밀 유지를 위해 모든 전환율에 임의의 상수를 곱했다고 밝힌다. 따라서 0.079%나 0.033%포인트를 실제 구매율이나 실제 추가 구매자 수로 읽으면 안 된다. lift는 $ATT/(\text{노출자 전환율}-ATT)$로 계산하므로 분자와 분모에 같은 상수가 곱해져 이 척도 변경의 영향을 받지 않는다. 신뢰구간은 원문 주석대로 부트스트랩으로 구했다.

Study 4 행으로 앞 절의 계산을 다시 따라가 보자. Table 3의 ITT 0.012를 노출률 0.37로 나누면 $0.012/0.37\approx0.032$이고, 반올림 전 값으로 계산한 표의 ATT는 0.033이다. 노출자가 광고를 보지 않았을 때의 전환율은 $0.079-0.033=0.046$이며 원문 각주 20도 같은 값을 제시한다. test 비노출자의 0.025를 7절의 $m_{10}$으로 두면 통제집단 전환율은 $0.37\times0.046+0.63\times0.025\approx0.033$이 되고, Table 3의 통제집단 값 0.033과 맞는다. 앞 절의 분해식이 실제 숫자에서도 성립한다는 확인이다.

Study 9는 이 표에서 가장 많은 것을 알려 주는 행이다. test 노출자의 전환율 2.105%는 비노출자 0.052%의 약 40배다. 그런데 실험으로 구한 ATT는 0.049%포인트이고 lift는 $0.049/(2.105-0.049)\approx2.4\%$에 그친다. 노출자와 비노출자의 큰 격차는 거의 전부 누가 광고를 보게 되었는가의 차이, 곧 선택에서 나온 것이다. 노출률이 6.6%로 매우 낮아 타기팅이 구매 가능성이 높은 소수를 골라낼 여지가 컸다는 것이 원문의 추측이다.

Study 12와 Study 4를 나란히 놓으면 절대효과와 상대효과가 다르게 움직이는 모습이 보인다. Study 12의 ATT는 0.078%포인트로 Study 4의 0.033보다 크지만, 기초 전환율이 높아 lift는 1.2%에 불과하다. 반대로 Study 5는 광고가 없었을 노출자의 전환율이 $0.055-0.045=0.010$으로 매우 낮아서 lift가 449.6%까지 커졌다. 분모가 작은 비율은 그 분모의 추정 오차에도 민감하다. 이 연구의 신뢰구간 [306, 761]도 넓고 비대칭이다.

Study 13에서는 비노출자의 전환율(0.309%)이 노출자(0.187%)보다 오히려 높고 ATT도 −0.033%포인트다. 단순 비교가 광고효과를 늘 부풀린다고 단정할 수 없는 사례다. 다만 이 lift의 신뢰구간 [−35, 20]은 0을 포함하므로 광고가 구매를 줄였다고 결론 내릴 수도 없다.

신뢰구간이 0을 포함하는 연구는 S2, S7, S8, S9, S10, S13의 여섯 개다. 이는 구매 결과가 있는 14개 연구 중 6개가 5% 수준에서 유의한 lift를 내지 못했다는 원문 본문의 서술과 맞는다. 원문은 이 중 두 개가 10% 수준에서는 유의하다고 적고, Study 9를 그중 하나로 명시한다.

이 표로 말할 수 있는 것은 캠페인별 실험효과의 크기와 불확실성, 그리고 노출자와 비노출자의 원자료 격차가 실험효과보다 훨씬 클 수 있다는 사실이다. 이 표만으로 관측자료 방법의 편향을 판단할 수는 없다. 그 비교는 Figure 10과 13절 뒤의 Table 6에서 이루어진다. 캠페인 사이에 lift가 다른 이유(업종, 예산, 소재 등)도 이 표로는 설명할 수 없고, 원문도 연구 수가 적어 편향과 관련된 캠페인 특성을 찾지 못했다고 밝힌다. 이윤 자료가 없으므로 광고 수익률도 계산할 수 없다. 표의 두 전환율로 단순 비교 lift를 만들면 Study 4는 $(0.079-0.025)/0.025\approx216\%$가 되는데, 원문이 보고한 단순 비교 lift 316%와 맞지 않는다. 원문은 이 차이를 설명하지 않으므로 이 문서도 맞추지 않는다. 이 불일치는 13절 Table 6 해설에서 다시 다룬다.
...
### 원문 Table 5: Study 4 노출자와 비노출자의 평균 특성

지금까지 노출 선택을 개념으로 설명했다. 원문 Table 5는 그 선택이 실제 캠페인에서 관측 특성의 차이로 어떻게 나타나는지 보여 준다. 대상은 Study 4의 test 집단이며, 매칭이나 가중을 하기 전의 단순 평균이다. 원문 Table 5(PDF 파일 기준 21쪽, 인쇄 213쪽)의 11개 행을 모두 옮겼다.

| 변수 | 비노출자 | 노출자 |
|---|---|---|
| Age | 26.4 | 29.3 |
| Female | 88% | 96% |
| Facebook age | 2,202 | 2,242 |
| No. of friends | 618 | 608 |
...
*출처: Gordon 등 (2019) Table 5 "Mean by Exposure" 전체, PDF 21쪽. 변수 이름은 원문 표기를 그대로 두었다.*

각 칸은 해당 집단의 평균이다. 비율로 표시된 행(Female, Married, Single, Phone A-C)은 그 특성을 가진 사용자의 비율이고, 나머지는 평균값이다. Age는 나이(세), Facebook age는 원문 Table 2 정의상 가입 후 경과 일수다. Facebook web과 Facebook mobile은 Table 5 자체에 단위가 적혀 있지 않다. Table 2에는 최근 7일 또는 28일 중 데스크톱이나 모바일로 접속한 날 수를 세는 변수가 있고, 모바일 값 24.8과 26.8이 28을 넘지 않으므로 28일 기준 접속일수일 가능성이 크다. 다만 원문이 둘의 대응을 명시하지 않았으므로 확정할 수는 없다. Phone A-C는 휴대전화 운영체제를 익명 처리한 범주로 보인다.

대표 수치를 읽어 보자. 여성 비율은 비노출자 88%, 노출자 96%로 약 8%포인트 차이가 난다. 원문 본문이 예로 든 바로 그 행이다. 여성 비율이 두 집단 모두 높은 것은 캠페인의 타깃 설정 때문으로 보이며, 여기서 볼 것은 같은 타깃 안에서도 노출 여부에 따라 구성이 다시 갈린다는 점이다. 노출자는 평균 2.9세 더 많고(26.4세 대 29.3세), 기혼 비율은 8%에서 19%로 두 배 이상 높다. 친구 수는 오히려 노출자가 10명 적다(618명 대 608명). 데스크톱 접속은 노출자가 적고(5.1 대 4.0) 모바일 접속은 노출자가 많다(24.8 대 26.8). 원문은 이를 두고 노출자와 비노출자 사이에 상당한 공변량 불균형이 있다고 정리한다.

이 표가 보여 주는 것은 무작위로 배정된 test 집단 안에서도 실제 노출자는 비노출자와 다른 사람들이라는 사실이다. 2절에서 test 안의 노출자만 골라 비교하면 무작위성의 보호를 잃는다고 한 설명이 실제 자료에서 확인된다. 이 표에는 표준오차나 집단별 표본 크기가 없어 각 차이의 정밀도는 판단할 수 없다. 원문은 차이의 크기를 볼 때 t 통계량보다 표준화 차이가 낫다고 보고 Figure 6에서 따로 제시한다. 어떤 변수가 구매를 얼마나 바꾸는지도 이 표에 없으므로, 불균형이 광고효과를 어느 방향으로 얼마나 치우치게 하는지 계산할 수 없다. 매칭 전 값이라는 점도 기억해야 한다. 조정 후 균형은 Figure 5와 Figure 6에서 확인한다. 또한 이 표는 Study 4 한 캠페인의 결과이며 다른 캠페인의 불균형 방향이 같다는 근거는 아니다. 관측 가능한 차이가 이 정도라면 관측되지 않은 구매 의도의 차이도 있을 수 있다는 점이 11절 논의의 출발점이다.
...
### 원문 Table 6: 층화 후에도 선택편향은 얼마나 남는가

민감도 분석을 캠페인별로 비교하려면, 관측 변수만으로 이미 편향을 얼마나 줄였는지부터 알아야 한다. 원문은 이를 brr(bias reduction ratio)로 요약한다. 이름과 달리 실제 식이 계산하는 것은 없앤 편향의 비율이 아니라, 조정 전 편향 중 조정 후에도 남은 비율이다.

$$
brr=\frac{|STRAT\ lift-RCT\ lift|}{|EU\ lift-RCT\ lift|}.
$$

분모는 노출자와 비노출자를 그대로 비교했을 때(EU)의 lift와 실험 lift의 거리다. 실험효과를 기준으로 빼 주었으므로, 단순 비교의 격차 전체가 아니라 그 비교에 섞인 선택편향의 크기를 나타낸다. 분자는 성향점수 층화(STRAT) 후에도 남은 거리다. brr이 작을수록 관측 변수로 격차의 많은 부분을 이미 줄였다는 뜻이다. 원문 Table 6(PDF 파일 기준 27쪽, 인쇄 219쪽)은 구매 결과 14개 연구를 brr 순으로 정렬했으며, 아래에 행과 열을 모두 옮겼다.

| 연구 | 결과변수 | EU lift (%) | STRAT lift (%) | RCT lift (%) | brr (%) |
|---|---|---|---|---|---|
...
| 9 | Checkout | 4,074 | 1,724 | 2.4 | 42 |
| 5 | Checkout | 678 | 306 | 450 | 63 |

*출처: Gordon 등 (2019) Table 6 "Summary of Bias Reduction Through Stratification" 전체, PDF 27쪽.*

세 lift 열과 brr 열의 단위는 %다. RCT lift 열은 앞의 Table 4 값을 반올림한 것이다. STRAT lift가 어떤 변수 조합을 썼는지는 표에 적혀 있지 않지만, Study 4의 99가 Figure 8에서 네 변수군을 모두 쓴 STRAT4 값과 같으므로 전체 변수를 사용한 층화로 읽는 것이 자연스럽다.

Study 4로 계산을 확인하면 $|99-73|/|316-73|=26/243\approx0.107$로 표의 11%가 나온다. 단순 비교가 만든 243%포인트의 격차 중 약 89%를 층화가 없앴고 26%포인트가 남았다는 뜻이다. 원문 Figure 11에서는 이 정도 잔여편향이 인구조사 변수 수준의 약한 미관측 변수로도 설명될 수 있었다. Study 9는 $|1724-2.4|/|4074-2.4|\approx0.42$다. 층화로 격차의 절반 이상을 줄였지만 남은 lift가 1,724%로 실험값 2.4%의 700배를 넘는다. 원문은 이 연구에서 미관측 변수가 현재 관측 변수의 5-10배 설명력을 가져야 한다고 보고한다.

brr이 작다고 해서 추정이 실험값에 가깝다는 뜻은 아니다. Study 2의 brr은 10%로 작지만 남은 STRAT lift 37%는 실험 lift 1.3%의 약 28배다. 원문은 이를 두고 마지막 구간이 가장 어려울 수 있다고 적는다. Study 15와 Study 10은 brr이 12-13%인데 STRAT lift의 부호가 음수(−13, −15)로 실험값(2.4, 2)과 반대다. brr은 절댓값으로 계산하므로 방향이 틀린 경우도 작은 값을 받을 수 있다. Study 5에서는 층화가 실험값 450%보다 작은 306%를 내어 과소추정 쪽으로 넘어갔고, brr은 63%로 가장 크다.

이 표로 말할 수 있는 것은 관측 변수로 조정한 뒤에도 남는 선택편향의 비율이 캠페인마다 0.4%에서 63%까지 크게 다르다는 사실, 그리고 그 비율이 클수록 편향을 없애는 데 더 강한 추가 정보가 필요하다는 원문의 관찰이다. 반면 이 표에는 신뢰구간이 없어 STRAT lift와 RCT lift의 차이가 통계적으로 유의한지는 알 수 없다. 그 검정은 Figure 8-10에 표시된다. 층화 한 가지 방법과 구매 결과에 대한 요약이므로 다른 방법이나 registration, page view 결과로 넓혀 읽을 수도 없다.

원문 자체의 불일치도 몇 가지 있어 수정하지 않고 알려 둔다. 인쇄 217쪽(PDF 25쪽) 본문은 Study 9의 단순 비교 효과를 4,047%로 적지만, Table 6과 Figure 11은 4,074로 표기한다. 본문 쪽에서 숫자 자리가 바뀐 오타로 보이지만, 원문 안에서 어느 쪽이 맞는지 확정할 근거는 없다. Study 7의 STRAT lift는 Table 6에서 35지만 Figure A.1(PDF 29쪽)의 제목은 STRAT=−35.3이다. 표의 brr 25%는 $|35-2.7|/|131-2.7|\approx0.25$로 양수 35와 맞고, −35.3을 넣으면 약 30%가 된다. 어느 쪽이 옳은지는 원문만으로 판단할 수 없다. 끝으로 EU lift 열은 Table 4의 노출자 전환율을 비노출자 전환율로 나눈 값에 100을 곱한 수와 거의 일치한다. 예를 들어 Study 4는 $0.079/0.025=3.16$, Study 13은 $0.187/0.309\approx0.61$, Study 10은 $0.127/0.092\approx1.38$이다. 식 (13)의 정의대로 증가율 $(E[Y(1)]-E[Y(0)])/E[Y(0)]$을 계산하면 각각 216%, −39%, 38%가 되어야 한다. 원문은 이 차이를 설명하지 않는다. 따라서 EU lift 열을 증가율로 읽을지 비율로 읽을지는 확정할 수 없으며, 이에 따라 brr의 분모도 달라질 수 있다. 7절 Table 4 해설과 마지막 출처 절에서 언급한 316% 재현 문제도 같은 원인일 수 있지만, 이 문서에서는 관찰로만 남긴다.
